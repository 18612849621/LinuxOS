...
| 特性             | `=`（递归赋值）              | `:=`（立即赋值）           |
| ---------------- | ---------------------------- | -------------------------- |
| **展开时机**     | 引用时动态展开（延迟）       | 定义时立即展开（静态）     |
| **依赖后续变量** | 支持（向前引用）             | 不支持                     |
| **性能**         | 较低（每次引用重新计算）     | 较高（仅计算一次）         |
| **典型用途**     | 动态配置、依赖复杂逻辑的变量 | 固定路径、编译选项等静态值 |

## gcc

| 符号                   | 作用                                |
| ---------------------- | ----------------------------------- |
| ​-fno-builtin          | 引用时动态展开（延迟）              |
| -c                     | 仅编译不链接，生成.o目标文件        |
| -Wall                  | 启用所有警告信息                    |
| -m32                   | 编译 32 位内容                      |
| ​​-Wstrict-prototypes  | 函数原型未明确参数类型（如 func()） |
| ​​-Wmissing-prototypes | 全局函数定义前未声明原型            |
| ​​-W                   | 高风险潜在错误（如类型错误）        |
